...
argument-hint: "[tabId(推奨/数値) もしくは urlContains] [指示/テキスト(任意)] (例: 1234 修正して)"
...
UI要素にバグを見つけて拡張でメモを残したあと、**`/bag-memo <tabId> [指示/テキスト(任意)]` と打つだけ**で、そのメモと対象要素を
**画像トークンなし**で取得し、コード上の `file:line` に対応づけて**UIメモ取得票**を出します。
通常は読み取り（取得＋特定）専用ですが、ユーザーが後ろに具体的な追加指示（例: 「修正して」「ここを開いて」「diffを見せて」など）を付与した場合は、取得したメモや要素、ソース箇所の情報を用いて、その指示（編集やコマンド実行など）を実行してください。

契約は1本の矢印: **メモ / MEMO → 対象要素 / ELEMENT → ソース / SOURCE(file:line)**。追加指示がない場合は、ここで止まり、修正は委譲します。
...
- **直さない(指示がない場合)**: 追加の指示がない場合は勝手にコードを変更しません。また、Edit/Write系の編集ツールは `allowed-tools` に定義していないため、指示に基づいて修正を行う際にはユーザーの承認プロンプトが表示されます。
