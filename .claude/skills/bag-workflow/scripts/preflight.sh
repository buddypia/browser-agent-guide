...
# --- 3) Claude Code に bag_page_feedback / bag_visual_feedback MCP が登録されているか / registered? ----
# claude mcp list は MCP の health check で数秒かかることがある。失敗は absent 扱い。
# 接頭辞(alias)はユーザが登録時に決めた名前なので、新旧どちらでも検出する。
...
echo "mcp      : $mcp   (Claude Code の bag_page_feedback / 旧 bag_visual_feedback 登録)"
