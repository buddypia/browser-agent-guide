...
# --- 3) Claude Code に bag_page_feedback MCP が登録されているか / registered? ----
# claude mcp list は MCP の health check で数秒かかることがある。失敗は absent 扱い。
# 接頭辞(alias)はユーザが登録時に決めた名前。推奨は bag_page_feedback（旧 bag_visual_feedback は付け替え済み前提）。
if command -v claude >/dev/null 2>&1 && claude mcp list 2>/dev/null | grep -qi 'bag_page_feedback'; then
  mcp="registered"
else
  mcp="absent"
fi
...
echo "mcp      : $mcp   (Claude Code の bag_page_feedback 登録)"
