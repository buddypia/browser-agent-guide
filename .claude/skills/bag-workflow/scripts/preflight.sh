...
# --- 3) Claude Code に visual_feedback MCP が登録されているか / registered? ----
# claude mcp list は MCP の health check で数秒かかることがある。失敗は absent 扱い。
if command -v claude >/dev/null 2>&1 && claude mcp list 2>/dev/null | grep -qi 'visual_feedback'; then
  mcp="registered"
else
  mcp="absent"
fi
...
echo "mcp      : $mcp   (Claude Code の visual_feedback 登録)"
