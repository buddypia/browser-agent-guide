...
```bash
cd "$(git rev-parse --show-toplevel)/daemon"   # ① daemon フォルダへ移動 / go to the folder
...
それも無ければ: `node "$(git rev-parse --show-toplevel)/daemon/scripts/probe.mjs" ~/Downloads/ai-inbox --url <断片>`
...
> **「メモを残す」だけでは届かない / a memo must be SENT first(canonical — 他の参照/スキルはここへリンクする)**:
> メモは「メモを残す」を押した時点ではページ(`chrome.storage.local`)に保存されるだけで、上の送信
> (または Options で daemon + autoSync を両方 ON。**どちらも既定 OFF**)まで daemon には1件も届かない
> (既定 `memory` モードでは `annotation.json` ファイルすら作られず RAM 保持のみ。disk/hybrid や
> daemon-OFF フォールバックのみファイル化)。**「見つからない」の最頻原因は tabId/urlContains 違いではなく
> この未送信**。再スコープの前に、まず「送ったか?」を疑う。
> A memo left on the page is NOT in the inbox until you press "Send to AI"/画像でAIへ送る (or enable
> daemon+autoSync, both default OFF). A "not found" is most often an UNSENT memo, not a wrong
> tabId/urlContains — suspect this before re-scoping.
...
```bash
node "$(git rev-parse --show-toplevel)/scripts/read-annotations-cdp.mjs" --kind note
```
