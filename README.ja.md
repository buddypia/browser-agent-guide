...
表示中のページに**補足・お描き（要素を円・四角・矢印・ペンで囲む）をクリックだけで付けられる** Chrome 拡張機能です（目印・合図ボタンは AI が動詞レジストリ経由で付けます）。
...
> クリックで付けるのは **💬補足（AIへの指示）** と **🖍お描き（要素を円・四角・矢印・ペンで囲む）**。
> **📌目印（要素に決定的な名前を付ける）** / **🔘合図ボタン** は AI が付ける種類です。
...
3. 出てきたフォームに **AIへの指示・補足** を1つ書いて保存
   - 種類選択は不要です。「ここは送信前に必ず確認する」のような AI 向けの内容だけを書きます
   - 保存すると対象要素は**赤い枠**で囲まれ、どこに補足を付けたかひと目で分かります
...
4. **完了** を押すと、囲んだ要素に対して **AIへの指示・補足** を1つ入力して保存（Escで終了）
...
ツールバー:
- **＋補足を付ける**: 要素をクリックして**補足（AIへの指示）**を1つ付ける注釈モード（対象は赤枠で囲まれる）
