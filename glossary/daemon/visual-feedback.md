...
api_refs:
  - { name: "MCP: get_latest_feedback_context (旧 get_latest_visual_feedback_context, deprecated alias)", spec: "daemon/src/server.js" }
  - { name: "MCP: get_latest_feedback_image (旧 get_latest_visual_feedback, deprecated alias)", spec: "daemon/src/server.js" }
db_refs: []
...
は deprecated エイリアス（同一ハンドラ）として当面残す。Claude Code 上の `bag_visual_feedback:`
接頭辞は**ユーザの MCP 登録 alias**でありサーバ名リネームでは変わらない（接頭辞の後ろのツール名だけ改名）。
