...
text-only(メモのみ同期; `image.shot` なし)の push は `annotation` があれば全モードで受理する。
その entry は `shotBuffer`/`shot.png` を持たず（`entry.shot` は空文字列）、materialize しても
annotation.json/memo.md だけが書かれる（`entryHasImage`=false。glossary/daemon/page-feedback.md 参照）。
