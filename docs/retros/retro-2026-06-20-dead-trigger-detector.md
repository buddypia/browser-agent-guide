...
- [x] Optional hardening: add per-guard contract tests (canonical-command-still-fires) for the command-gating hooks. Added `commit-guard.test.mjs` + `destructive-git-guard.test.mjs` (the two with zero coverage; both modified by PR #25) asserting deny/passthrough contracts on canonical commands — covering the regex-trigger-liveness vector the lint intentionally does not — and locking in PR #25's carve-out removal (`decide` ignores a stray `isCreatePrActive`; destructive patterns still block). `worktree-*-guard` already have `evaluate`-level tests via `worktree-review-report-guard.test.mjs`. (this PR)
