...
お描き (page feedback) を Claude に届ける常駐 daemon (内部名 `bag-page-feedback`) と、その MCP ツールの使い方。
...
| ツール | 入力 | 返り値 | 用途 |
|---|---|---|---|
| `bag_page_feedback:list_feedback` | `{ limit?, urlContains?, titleContains?, tabId?, windowId? }` | text 一覧 (id・url・title・時刻・tab) | 候補を新しい順に列挙 |
...
## `source_branch=MCP`/`mcp=registered` は「呼べる」保証ではない(canonical — bag-workflow/bag-memo 共通)

preflight の `STATUS` 行が `source_branch=MCP`(または `mcp=registered`)でも、それは **この preflight を
実行したその瞬間**に別プロセスの `claude mcp list`/`codex mcp list` が疎通確認した結果にすぎない。
Claude Code は MCP サーバーへの接続を**この会話セッションが起動した時点**で確立し、途中で切断された時
だけ自動再接続する(最大5回) —— **起動した時点で接続できていなければ、daemon が後から立ち上がっても、
この会話セッションでは自動回復しない**。

よって `source_branch=MCP` でも、実際にツールを呼ぶ前に**必ず ToolSearch で `bag_page_feedback` 系ツール
(`mcp__bag_page_feedback__*`)が見つかるか確認**し、見つからなければ FILE 経路(`capture=yes` の場合)に
切り替えるか、`capture=no` なら「新しい会話セッションを開始してください」と日本語＋英語で案内する
(具体的な案内文は `fallbacks.md` の「MCP は registered/Connected のはずなのに ToolSearch で見つからない」節)。

## Claude Code に MCP を登録する (1回だけ)

💡 **簡易セットアップ**:
リポジトリのルートで以下のコマンドを実行することで、依存関係のインストールから Claude Code への MCP 登録までを対話形式でまとめてセットアップできます。
```bash
make setup.mcp
```

または、以下の手動コマンドを実行します (そのままコピペでよい。フラグや URL は変えない)。
...
```
環境変数 `BAG_PF_INBOX` / `BAG_PF_PORT` / `BAG_PF_HOST` で上書き可（旧 `BAG_VF_*` も後方互換で読む）。
...
```bash
node "$(git rev-parse --show-toplevel)/daemon/scripts/probe.mjs" ~/Downloads/ai-inbox --url <部分一致>
```

## annotation.json スキーマ (実物 / schema `bag.page-feedback/v1`)

> 現行は `bag.page-feedback/v1` (各 item に `html`/`a11y` を含む)。旧 `bag.visual-feedback/v1` も同一構造で、
> daemon はスキーマ名の literal で分岐しないのでそのまま読める。さらに古いキャプチャは `…/v0` や
