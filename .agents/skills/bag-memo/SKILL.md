...
> 再スコープ前にまず「送ったか?」を疑う。正本の案内文・回復策 → `../bag-workflow/references/fallbacks.md`
> 「お描き／メモが見つからない」節(canonical / A memo is NOT in the inbox until sent)。
...
> **`source_branch=MCP` は「呼んでよい」許可であって「呼べる」保証ではない**。ツールを呼ぶ前に**必ず
> ToolSearch で `mcp__bag_page_feedback__*` が見つかるか確認**し、無ければステップ2の「MCP 未接続の場合」へ。
> (理由の正本 → `../bag-workflow/references/daemon-mcp.md`)
...
> **stale(鮮度切れ)分岐**: 「latest が古すぎます / stale」ブロックが返ったら、tabId 不一致ではなく**鮮度切れ**
> (再スコープでは直らない)。回復策は (a) 拡張で**いまの画面を再キャプチャ**、または (b) **freshness ガードの
> 無い by-id** で取り直す(`list_feedback({ tabId })` で `id` → `get_feedback_context({ id })`)。
> 仕組みの正本 → `references/tabid-addressing.md`。
