...
侧边栏还会显示当前操作目标 Chrome 标签页（`tabId` / `windowId` / 标签页位置）。页面反馈截图会把同一份标签页信息保存到 `annotation.json` / `memo.md`，并通过 daemon MCP context 暴露出来；如果同一个 URL 在多个标签页中打开，可在 MCP 工具中传入 `tabId`（或 `windowId`）来区分。
...
**添加批注** 只会保存到扩展自身的 `chrome.storage.local`（键 `aiAdvisorAnnotations`）——在你按下
**以图片发给AI** 之前（或在设置中启用 `daemon` + `pageFeedback.autoSync`，两者默认均为 OFF）都不会
发送到 daemon/MCP。如果你写完批注后在发送前就关闭了标签页，`scripts/read-annotations-cdp.mjs` 可以
通过 Chrome DevTools Protocol 直接从浏览器中读回——无需 daemon，也无需重新截取：

```bash
# 需要先以带 remote-debugging 端口的方式启动 Chrome（安全的启动方法见下方文档）:
npm run read-annotations                         # 按最新优先打印所有批注/绘制
node scripts/read-annotations-cdp.mjs --kind note --scope example.com
```

完整设置步骤、参数说明，以及不使用 `chrome-devtools-mcp` 的原因（截至 v1.4.0，其
`--categoryExtensions` 在 attach 连接下不生效）见
...
`tabId` 是当前 Chrome 会话中的实时操作目标。页面反馈还会保存 `{tabId, windowId, index, active}`。它不是持久的浏览器历史 ID，但适合用来区分当前打开的同 URL 多标签页。
