...
export const DEFAULT_SETTINGS = {
  ...
  // 記録済みワークフローの自動実行設定。
  workflow: {
    allowIrreversibleClicks: false,
  },
  // 拡張UIの表示言語。既定は 'auto'(ブラウザ言語に追従、非対応は英語)。
  // サイドパネルから具体言語を選ぶと、その値が保存され以後優先される。
  ui: {
    language: 'auto', // 'auto' | 'en' | 'ko' | 'ja' | 'zh'
  },
  // ページフィードバックの常駐デーモン(Phase 1b)。有効時はお描き画像を WebSocket で push する
  // (無効/未到達時は chrome.downloads にフォールバック)。トークンはデーモン起動時の表示を貼る。
  daemon: {
    enabled: false,
    url: 'ws://127.0.0.1:8765/ws',
    token: '',
    // 保存先フォルダ。空欄なら OS の既定ダウンロードフォルダ。daemon 有効時はこの値を
    // WebSocket push で daemon に伝え、daemon が <saveDir>/ai-inbox を inbox に採用する。
    saveDir: '',
  },
  // メモ／お描きを保存操作なしでローカル daemon へ自動送信する。daemon の有効化
  // (daemon.enabled) が唯一のスイッチ（旧 autoSync トグルは廃止し daemon.enabled に一本化）。
  // (旧名 visualFeedback から改名。getSettings が旧キーを移行する)
  pageFeedback: {
    autoSyncDebounceMs: 1800,
  },
...
export async function getSettings() {
  ...
  return {
    ...
    // 旧キー visualFeedback で保存された設定を新キー pageFeedback へ移行して読む。
    pageFeedback: { ...DEFAULT_SETTINGS.pageFeedback, ...(stored.pageFeedback || stored.visualFeedback || {}) },
    sites: Array.isArray(stored.sites) ? stored.sites : [],
    recipes: stored.recipes && typeof stored.recipes === 'object' ? stored.recipes : {},
  };
}

/** 自動送信(メモの text-only / お描きの画像)の前提が揃っているか。daemon の有効化
 *  (enabled+url+token)を唯一のスイッチにする（旧 pageFeedback.autoSync トグルは廃止）。
 *  service-worker(自動送信ゲート)と sidepanel(UI 文脈適応)で共有する。 */
export function isAutoSyncReady(settings) {
  const d = settings?.daemon || {};
  return Boolean(d.enabled && d.url && d.token);
}
...
export async function patchSettings(patch) {
  ...
  const next = {
    ...
    workflow: { ...cur.workflow, ...(patch.workflow || {}) },
    ui: { ...cur.ui, ...(patch.ui || {}) },
    daemon: { ...cur.daemon, ...(patch.daemon || {}) },
    pageFeedback: { ...cur.pageFeedback, ...(patch.pageFeedback || {}) },
