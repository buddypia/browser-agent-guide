...
사이드 패널에는 현재 조작 대상인 Chrome 탭(`tabId` / `windowId` / 탭 위치)도 표시됩니다. 시각 피드백 캡처는 같은 탭 정보를 `annotation.json` / `memo.md`에 저장하고 daemon MCP context에도 노출하므로, 같은 URL을 여러 탭에서 연 경우 MCP 도구에 `tabId`(또는 `windowId`)를 넘겨 구분할 수 있습니다.
...
**이미지로 AI에 보내기**를 누르기 전까지는(또는 설정에서 `daemon` + `visualFeedback.autoSync`,
...
`tabId`는 현재 Chrome 세션 안의 실시간 조작 대상입니다. 시각 피드백은 `{tabId, windowId, index, active}`도 저장합니다. 영구적인 브라우저 기록 ID는 아니지만, 열려 있는 같은 URL의 여러 탭을 구분하는 식별자로 사용합니다.
