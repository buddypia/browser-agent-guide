...
사이드 패널에는 현재 조작 대상인 Chrome 탭(`tabId` / `windowId` / 탭 위치)도 표시됩니다. 페이지 피드백 캡처는 같은 탭 정보를 `annotation.json` / `memo.md`에 저장하고 daemon MCP context에도 노출하므로, 같은 URL을 여러 탭에서 연 경우 MCP 도구에 `tabId`(또는 `windowId`)를 넘겨 구분할 수 있습니다.
...
**메모 추가**는 확장 기능 자체의 `chrome.storage.local`(키 `aiAdvisorAnnotations`)에만 저장되며,
**이미지로 AI에 보내기**를 누르기 전까지는(또는 설정에서 `daemon` + `pageFeedback.autoSync`,
둘 다 기본값 OFF, 를 켜지 않는 한) daemon/MCP로 전송되지 않습니다. 메모를 남긴 탭을 전송 전에
닫아버렸더라도, `scripts/read-annotations-cdp.mjs`가 Chrome DevTools Protocol을 통해 브라우저에서
직접 읽어올 수 있습니다 — daemon도 재캡처도 필요 없습니다:

```bash
# 사전에 remote-debugging 포트로 Chrome을 실행해 두어야 합니다 (안전한 실행 방법은 문서 참고):
npm run read-annotations                         # 저장된 모든 메모/그림을 최신순으로 출력
node scripts/read-annotations-cdp.mjs --kind note --scope example.com
```

전체 설정 방법과 옵션, 그리고 `chrome-devtools-mcp`를 쓰지 않는 이유(v1.4.0 기준
`--categoryExtensions`가 attach 연결에서는 동작하지 않음)는
[docs/reading-annotations-via-cdp.md](docs/reading-annotations-via-cdp.md)에 있습니다.
...
`tabId`는 현재 Chrome 세션 안의 실시간 조작 대상입니다. 페이지 피드백은 `{tabId, windowId, index, active}`도 저장합니다. 영구적인 브라우저 기록 ID는 아니지만, 열려 있는 같은 URL의 여러 탭을 구분하는 식별자로 사용합니다.
...
- 기록 워크플로 자동 실행은 기본적으로 최종 확정/구매/삭제처럼 보이는 클릭을 보류합니다. Options에서 기록된 단계를 신뢰하도록 명시적으로 허용할 수 있습니다.
