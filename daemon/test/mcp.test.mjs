...
test('Codex#10334 パリティ: image を返す全分岐で structuredContent を載せない', async () => {
  ...
  await withClient(async (client) => {
    ...
    assertImageHasNoStructured(single, 'get_latest_visual_feedback single');
    ...
    assertImageHasNoStructured(byId, 'get_visual_feedback by-id');
    ...
    assertImageHasNoStructured(filtered, 'get_latest_visual_feedback filtered');
  });
...
test('get_visual_feedback: inline(webp) があれば webp を返し、file_path はフル解像度のまま、structuredContent 無し', async () => {
...
test('get_visual_feedback: inline 無しで full-res PNG が予算超過なら image を omit（text-only, structuredContent 無し）', async () => {
