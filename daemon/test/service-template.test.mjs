...
test('launchdPlist: node+script+args を ProgramArguments に並べ、常駐キーを含む', () => {
  const plist = launchdPlist({
    ...
    args: ['--port', '8765'],
    env: { BAG_VF_TOKEN: 'tok&en' },
    logDir: '/home/u/Library/Logs',
  });
  assert.match(plist, /<key>Label<\/key>\s*<string>com\.buddypia\.bag-vf-daemon<\/string>/);
  ...
  // env はエスケープされて入る
  assert.match(plist, /<key>BAG_VF_TOKEN<\/key>\s*<string>tok&amp;en<\/string>/);
  assert.match(plist, /bag-vf-daemon\.out\.log/);
  assert.match(plist, /bag-vf-daemon\.err\.log/);
});
...
test('servicePlan darwin: LaunchAgents の plist と launchctl コマンド', () => {
  ...
  // ログ先が home 配下
  assert.match(plan.content, /\/Users\/me\/Library\/Logs\/bag-vf-daemon\.out\.log/);
});
...
test('servicePlan linux: systemd user unit と systemctl コマンド', () => {
  ...
  assert.match(plan.loadCmd, /systemctl --user enable --now bag-vf-daemon\.service/);
});
