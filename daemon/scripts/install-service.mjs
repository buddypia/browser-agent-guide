...
function parse(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (a === '--token') out.env.BAG_PF_TOKEN = argv[(i += 1)];
...
function main() {
  ...
  try {
    ...
    process.stderr.write(`[bag-pf] ${e.message}\n`);
    process.stderr.write('[bag-pf] 手動常駐化: nohup/pm2/tmux 等で `node src/index.js` を起動してください。\n');
    return 1;
  }

  if (!opts.write) {
    process.stdout.write(plan.content);
    process.stderr.write(`\n[bag-pf] 上記を ${plan.unitPath} に保存し、次で常駐化:\n`);
    process.stderr.write(`[bag-pf]   ${plan.loadCmd}\n`);
    process.stderr.write(`[bag-pf] そのまま書き出すには --write を付けて再実行。\n`);
    return 0;
  }
  ...
  try {
    ...
    process.stderr.write(`[bag-pf] 書き出し失敗: ${e.message}\n`);
    return 1;
  }
  process.stderr.write(`[bag-pf] ユニットを書き出しました: ${plan.unitPath}\n`);
  process.stderr.write(`[bag-pf] 常駐化(load)は手動で実行してください:\n`);
  process.stderr.write(`[bag-pf]   ${plan.loadCmd}\n`);
  process.stderr.write(`[bag-pf] 状態確認: ${plan.statusCmd}\n`);
  process.stderr.write(`[bag-pf] 停止/解除: ${plan.unloadCmd}\n`);
  return 0;
}
