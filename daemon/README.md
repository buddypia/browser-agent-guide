# Browser Agent Guide ページフィードバック デーモン（Phase 1 / 受信側 + 消費側）
...
```bash
# token は起動時 stderr / ~/.bag-pf/token / healthz で確認
...
では一切発火しない**。鮮度窓/曖昧検知窓は `--latest-window-min` / `BAG_PF_LATEST_WINDOW_MIN`（分）で調整可。
...
（または `BAG_PF_RETENTION=on`）で、デーモンが古い entry を `<inbox>/done/` へ**退避**する
...
BAG_PF_RETENTION=on node src/index.js
```
...
`BAG_PF_TOKEN` として渡る。`--write` 後に表示される load コマンドを実行して常駐化する:

```bash
# macOS（~/Library/LaunchAgents/com.buddypia.bag-pf-daemon.plist）
launchctl load -w ~/Library/LaunchAgents/com.buddypia.bag-pf-daemon.plist
# Linux（~/.config/systemd/user/bag-pf-daemon.service）
systemctl --user daemon-reload && systemctl --user enable --now bag-pf-daemon.service
```
...
1. デーモンを起動し、stderr に出る `token: …` をコピー（保存先は `~/.bag-pf/token`）。
2. 拡張の設定（オプションページ）→「ページフィードバック デーモン」:
   - 「デーモンへ送る」を ON
...
- loopback バインドのみ。**WebSocket は秘密トークンで認証**（クエリ `?token=`、定数時間比較、
  ...
  接続しうる）ため、トークンで拡張だけを許可する。トークンは `~/.bag-pf/token`（0600。無ければ旧 `~/.bag-vf/token` も後方互換で読む）。
