...
- 5 つの MCP ツールを公開（接頭辞 `bag_visual_feedback:` は**登録時に付けた alias** 例。任意の名前でよい）:
  ...
  > なお Claude Code/Codex 等で打つ接頭辞（例 `bag_visual_feedback:`）は**ユーザが登録時に決めた alias**で、サーバ名の
  > リネームでは変わらない。好みで `bag_page_feedback` 等に付け替えてよい（`claude mcp remove <旧> && claude mcp add <新> …`）。
...
```
bag_visual_feedback:get_latest_feedback_context({ urlContains: "example.com" })   # その URL を含む最新だけ
bag_visual_feedback:list_feedback({ titleContains: "ダッシュボード" })
bag_visual_feedback:get_latest_feedback_context({ urlContains: "example.com", tabId: 123 })  # 同じURLの特定タブ
```
...
3. CLI に MCP を登録して、こう頼む:
   ...
   → CLI が `bag_visual_feedback:get_latest_feedback_context` を呼び、`@agent:` / selector / testid を先に読む。
   ...
   `bag_visual_feedback:get_latest_feedback_image` を呼び、返った image を vision 解釈する。
