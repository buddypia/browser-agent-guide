...
- 5 つの MCP ツールを公開:
  - `bag_visual_feedback:list_visual_feedback` — 一覧（id・取得元 url/title/tab 付き）
...
メモリで保持して `get_latest_visual_feedback_context` / image ツールに返せる。image を読めない CLI のために
...
- `memory`（**既定**）: WS push はメモリ保持のみで、`<Downloads>/ai-inbox/` を作らない。
  ...
  再起動を跨ぐ永続化・履歴・`list_visual_feedback`・retention は持たない（必要なら hybrid/disk）。
...
bag_visual_feedback:get_latest_visual_feedback_context({ urlContains: "example.com", tabId: 123 })  # 同じURLの特定タブ
```
...
3. CLI に MCP を登録して、こう頼む:
   ...
   → 同じ URL のタブが複数ある時は、サイドパネルに表示された `tabId` を `get_latest_visual_feedback_context` / `get_latest_visual_feedback` に渡す。
